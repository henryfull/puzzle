# Script para crear un ScrollContainer personalizado con soporte táctil mejorado
# Este script se utiliza para crear una escena de ScrollContainer personalizada
...
# Propiedades exportadas para configurar el comportamiento táctil
@export var touch_scroll_speed: float = 1.0
@export var touch_scroll_inertia: float = 0.9
@export var touch_scroll_deadzone: int = 10
@export var touch_hold_delay: float = 0.2  # Tiempo en segundos para considerar un toque como "mantenido"
...
func _ready():
	...
	print("TouchScrollContainer inicializado: ", name)
...
func _gui_input(event):
	# Manejar eventos táctiles dentro del ScrollContainer
	if event is InputEventScreenTouch:
		_handle_touch_event(event)
	elif event is InputEventScreenDrag:
		_handle_drag_event(event)
	elif event is InputEventMouseButton:
		# Emular eventos táctiles desde el ratón para pruebas en escritorio
		if not OS.has_feature("mobile") and not OS.has_feature("ios") and not OS.has_feature("android"):
			...
			touch_event.index = 0  # Simular el primer dedo
			_handle_touch_event(touch_event)
	elif event is InputEventMouseMotion:
		# Emular eventos de arrastre desde el ratón para pruebas en escritorio
		if not OS.has_feature("mobile") and not OS.has_feature("ios") and not OS.has_feature("android"):
			if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
				var drag_event = InputEventScreenDrag.new()
				drag_event.position = event.position
				drag_event.relative = event.relative
				drag_event.velocity = event.velocity
				drag_event.index = 0  # Simular el primer dedo
